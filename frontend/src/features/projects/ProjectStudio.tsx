...
export function ProjectStudio() {
  ...
  async function openProject(project: ProjectResponse, signal?: AbortSignal) {
    ...
    try {
      ...
      if (!signal?.aborted) {
        setMessage(error instanceof Error ? error.message : "Could not load song metadata.");
      }
    }
  }
  ...
  async function removeCurrentProject() {
    if (!editor.id || !window.confirm(`Delete “${editor.title}”? Generated media is not silently removed.`)) {
      return;
    }
  ...
  return (
    <main className="studio-shell">
      <ProjectSidebar
        projects={projects}
        selectedProjectId={editor.id}
        loading={loadState === "loading"}
        error={loadState === "error" ? loadError : undefined}
        onCreateNew={beginNewProject}
      ...
      <section className="studio-workspace">
        ...
        <div className="mode-tabs" role="tablist" aria-label="Editor mode">
          ...
          <button role="tab" aria-selected="false" className="mode-tab" type="button" disabled>
            Advanced <span>later</span>
          </button>
          <button role="tab" aria-selected="false" className="mode-tab" type="button" disabled>
            Expert / Custom <span>later</span>
          </button>
        </div>
