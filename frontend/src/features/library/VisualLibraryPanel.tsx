...
export function VisualLibraryPanel({ onChanged }: VisualLibraryPanelProps) {
  ...
  async function toggleAssetFavorite(asset: AssetLibraryResponse) {
    setBusy(true);
    try {
      await updateAssetLibrary(asset.id, { name: asset.name, tags: asset.tags, isFavorite: !asset.isFavorite, sourceDescription: asset.sourceDescription });
      await refresh();
    } catch (error) {
  ...
  async function removeAsset(asset: AssetLibraryResponse) {
    ...
    try {
      ...
      if (!result.deleted) setMessage(`Cannot remove “${asset.name}”: referenced by ${result.referencingIds.length} library item(s).`);
      else {
        await refresh();
  ...
  return (
    <section className="library-panel" aria-labelledby="library-heading">
      <div className="section-heading"><div><span>06</span><h2 id="library-heading">Visual Library</h2></div><p>Reusable Characters, Styles, Locations and reference media shared across projects.</p></div>
          ...
          <div className="library-card-grid">
            {filtered.map((item) => (
              <article key={item.id} className="library-card">
                ...
                <strong>{item.name}</strong><p>{item.description}</p><small>{item.tags.join(" · ") || "No tags"}</small>
        ...
        <div className="asset-grid">
          {assets.map((asset) => (
            <article className="asset-card" key={asset.id}>
              <div className="asset-preview">{asset.previewMediaAssetId ? <Image src={getAssetPreviewUrl(asset.id)} alt="" width={240} height={150} unoptimized /> : <span>No preview</span>}</div>
...
function buildRequest(editor: EditorState, existing?: VisualLibraryResponse): VisualLibraryUpsertRequest {
  const shared = { kind: editor.kind, name: editor.name, description: editor.description, tags: split(editor.tags), isFavorite: editor.favorite, assetEntryIds: editor.assetIds, character: null, style: null, location: null } satisfies VisualLibraryUpsertRequest;
...
function toEditor(item: VisualLibraryResponse): EditorState { return { id: item.id, kind: item.kind, name: item.name, description: item.description, tags: item.tags.join(", "), favorite: item.isFavorite, assetIds: item.assetEntryIds, appearance: item.character?.appearanceDescription ?? "", forbidden: item.character?.forbiddenChanges.join(", ") ?? "", outfits: item.character?.outfits.map((outfit) => outfit.name).join(", ") ?? "", prompt: item.style?.prompt ?? "", camera: item.style?.cameraCharacteristics ?? "", lighting: item.style?.lightingCharacteristics ?? item.location?.lighting ?? "", animation: item.style?.animationCharacteristics ?? "", environment: item.location?.environmentDescription ?? "", constraints: item.location?.constraints.join(", ") ?? "", weather: item.location?.weather ?? "", timeOfDay: item.location?.timeOfDay ?? "" }; }
